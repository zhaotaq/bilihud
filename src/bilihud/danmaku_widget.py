...
from .ai_copilot_service import AICopilotService
from .ai_teleprompter_widget import AITeleprompterWidget
...
from .live_stats_dialog import LiveStatsDialog
from .live_stats_tracker import LiveStatsTracker
...
class ModernInputWidget(QWidget):
    ...
    def __init__(self, parent=None, placeholder="发送弹幕...", show_emoticon_button: bool = True):
        ...
        self.emoticon_btn.setFixedSize(30, 28)
        ...
        self.send_btn.setFixedSize(50, 28)
    ...
    def get_html_for_message(self, message) -> str:
        if isinstance(message, web_models.DanmakuMessage):
            ...
            <style>
                ...
                .user {{ color: {user_color}; font-weight: bold; font-family: 'Segoe UI', 'Microsoft YaHei'; font-size: 15px; }}
                .colon {{ color: white; font-family: 'Segoe UI', 'Microsoft YaHei'; font-size: 15px; }}
                .content {{ color: white; font-family: 'Segoe UI', 'Microsoft YaHei'; font-size: 17px; font-weight: 600; line-height: 140%; }}
                .reply {{ color: #FF79C6; font-family: 'Segoe UI', 'Microsoft YaHei'; font-size: 16px; font-weight: 700; }}
                .emoticon {{ vertical-align: middle; }}
                body, p {{ line-height: 135%; margin: 2px 0; padding: 0; }} 
            </style>
            ...
            return f"""
            <style>
                .user {{ color: #FFD700; font-weight: bold; font-family: 'Microsoft YaHei'; font-size: 15px; }}
                .action {{ color: #FF66CC; font-family: 'Microsoft YaHei'; font-size: 15px; }}
                .gift {{ color: #FF66CC; font-weight: bold; font-family: 'Microsoft YaHei'; font-size: 16px; }}
                body, p {{ line-height: 135%; margin: 2px 0; padding: 0; }}
            </style>
            ...
            return f"""
            <style>
                .user {{ color: #AAAAAA; font-weight: bold; font-family: 'Microsoft YaHei'; font-size: 14px; }}
                .info {{ color: #AAAAAA; font-family: 'Microsoft YaHei'; font-size: 14px; }}
                body, p {{ line-height: 130%; margin: 1px 0; padding: 0; }}
            </style>
...
class DanmakuWidget(QWidget):
    ...
    def __init__(self, room_id: int = 0, sessdata: str = ''):
        ...
        self.ai_copilot_service = AICopilotService(
            ...
            knowledge_base=config.get("ai_knowledge_base", ""),
            vault_path=config.get("ai_vault_path", ""),
        )
        self.ai_copilot_service.suggestion_generated.connect(self.on_ai_suggestion)
    ...
    def init_ui(self):
        ...
        self.stats_btn = QPushButton("📊 榜单")
        self.stats_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.stats_btn.setStyleSheet(btn_style)
        self.stats_btn.setToolTip("打开直播间实时数据看板与打赏金榜")
        self.stats_btn.clicked.connect(self.open_live_stats)

        self.fullscreen_btn = QPushButton("🖥️ 铺满")
        self.fullscreen_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.fullscreen_btn.setStyleSheet(btn_style)
        self.fullscreen_btn.setToolTip("一键平铺填满竖屏副屏 (或快捷键 F11 / 双击顶部)")
        self.fullscreen_btn.setCheckable(True)
        self.fullscreen_btn.clicked.connect(self.toggle_fullscreen_fill)
        ...
        self.header_layout.addWidget(self.ai_btn)
        self.header_layout.addWidget(self.fullscreen_btn)
        self.header_layout.addStretch()
        self.header_layout.addWidget(close_btn)

        self.ai_teleprompter_card = AITeleprompterWidget(self)
        ...
        self.main_layout.addWidget(self.ai_teleprompter_card)
    ...
    def setup_tray_icon(self):
        ...
        self.tray_fullscreen_action = QAction("🖥️ 填满竖屏副屏 (F11)", self)
        self.tray_fullscreen_action.triggered.connect(lambda: self.toggle_fullscreen_fill())
        tray_menu.addAction(self.tray_fullscreen_action)
        ...
        self.tray_ai_config_action = QAction("⚙️ AI 直播智囊配置", self)
        self.tray_ai_config_action.triggered.connect(self.open_ai_config)
        tray_menu.addAction(self.tray_ai_config_action)
        ...
        self.tray_stats_action = QAction("📊 直播数据看板与打赏榜", self)
        self.tray_stats_action.triggered.connect(self.open_live_stats)
        tray_menu.addAction(self.tray_stats_action)
    ...
    def add_message(self, message, _from_buffer=False):
        ...
        if hasattr(self, "stats_tracker") and self.stats_tracker is not None:
            self.stats_tracker.process_message(message)

        if hasattr(self, "ai_copilot_service") and self.ai_copilot_service is not None:
            self.ai_copilot_service.process_message(message)

    def on_ai_suggestion(self, uname: str, question: str, suggestion: str):
        """当 AI 提词服务生成新的口语化台词时触发渲染"""
        if hasattr(self, "ai_teleprompter_card") and self.ai_teleprompter_card is not None:
            self.ai_teleprompter_card.set_suggestion(uname, question, suggestion)

    def toggle_ai_teleprompter(self, checked: bool):
        """切换副屏 AI 悬浮提词卡片的显示与隐藏"""
        if hasattr(self, "ai_teleprompter_card") and self.ai_teleprompter_card is not None:
            self.ai_teleprompter_card.setVisible(checked)

    def toggle_fullscreen_fill(self, checked: bool = False):
        """一键平铺填满竖屏副屏 (或从最大化恢复)"""
        if self.isMaximized() or self.isFullScreen():
            self.showNormal()
            if hasattr(self, "fullscreen_btn"):
                self.fullscreen_btn.setChecked(False)
                self.fullscreen_btn.setText("🖥️ 铺满")
        else:
            ...
            if hasattr(self, "fullscreen_btn"):
                ...
                self.fullscreen_btn.setText("🖥️ 还原")

    def mouseDoubleClickEvent(self, event):
        """双击标题栏快捷切换铺满/还原"""
        if event.button() == Qt.MouseButton.LeftButton:
            self.toggle_fullscreen_fill()
            event.accept()
    ...
    def open_ai_config(self):
        """打开 AI 智囊 API Key 与模型配置窗口"""
        if not hasattr(self, "_ai_config_dialog") or self._ai_config_dialog is None:
            self._ai_config_dialog = AIConfigDialog(self)
        self._ai_config_dialog.show()
        self._ai_config_dialog.raise_()
        self._ai_config_dialog.activateWindow()

    def open_mock_simulator(self):
        """打开开播前弹幕与视觉效果模拟测试控制面板。"""
    ...
    def open_live_stats(self):
        """打开直播间实时数据看板与打赏榜弹窗。"""
        if not hasattr(self, "_live_stats_dialog") or self._live_stats_dialog is None:
            self._live_stats_dialog = LiveStatsDialog(self)
        self._live_stats_dialog.show()
        self._live_stats_dialog.raise_()
        self._live_stats_dialog.activateWindow()
